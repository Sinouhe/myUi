...
const messages = stylelint.utils.ruleMessages(ruleName, {
  tooMany:
    'Only one root-level ".myui-*" block is allowed per file (plus optional :root).',
  invalidName: 'Root block must start with ".myui-".',
});

const oneRootBlock = stylelint.createPlugin(ruleName, function () {
  return (root, result) => {
    const rootRules = root.nodes.filter(
      (node) => node.type === "rule" && node.parent === root,
    );
    ...
    rootRules.forEach((rule) => {
      ...
      // Check .fs-* selectors
      if (/^\.myui-[a-z]/.test(rule.selector)) {
        fsBlocks.push(rule);
      } else {
